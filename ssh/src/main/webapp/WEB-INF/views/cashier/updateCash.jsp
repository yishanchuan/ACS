      ...
      <div class="row">
        <div class="col-xs-12 col-md-12">
        	<div class="box" style="text-align:center;">
        		<form method="post" action="${pageContext.request.contextPath}/cashier/updateCash?id=${user.id}">
        		<div class="box-body">
        			<label>编码:</label><input type="text" name="cashId" value="${cash.id}" readOnly><p>
        			<label>名称:</label><input type="text" name="name" value="${cash.name}"><p>
        			<label>币别:</label>
        			<select id="current" class="chosen-select" name="currentId">
            			<option></option>
            			<c:forEach items="${listCurrent}" var="c" varStatus="cStatus">
            				<option value="${c.id}" ${cash.current.id==c.id?'selected':' '}>${c.id}&nbsp;&nbsp;${c.name}</option>
            			</c:forEach>
        			</select><p>
        			<label>会计科目:</label>
        			<select id="accountingSubject" class="chosen-select" name="accountingSubjectId">
            			...
            			<c:forEach items="${listas}" var="as" varStatus="asStatus">
            				<option value="${as.id}" ${cash.accountingSubject.id==as.id?'selected':' '}>${as.id}&nbsp;&nbsp;${as.name}</option>
            			</c:forEach>
        			</select><p>
