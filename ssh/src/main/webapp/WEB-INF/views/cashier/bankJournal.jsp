      ...
      <div class="row">
        <div class="col-xs-12">
          <div class="box">
            ...
            <form method="post" action="${pageContext.request.contextPath}/cashier/searchBankJournal?id=${user.id}">
            	<div class="col-xs-2">
            	<select class="chosen-select" id="bank" name="bank" style="width:120px;">
            		<option></option>
            		<c:forEach items="${listBank}" var="b" varStatus="bs">
            			<option value="${b.id}" 
            				<c:if test="${empty bank}&&${bs.count==1}">selected</c:if>
            				<c:if test="${!empty bank}">${b.id==bank.id?'selected':''}</c:if>
            			>${b.id}-${b.name}</option>
            		</c:forEach>
            	</select>
            	</div>
              ...
              <table id="journal" class="table table-bordered table-striped">
                ...
                <tbody>
					<tr>
						...
						<td></td>
						<td></td>
						<td><input type="number" class="fulltd" id="initial" name="initial" value="${bank.balance}"></td>
						<td></td>
					</tr>
					<c:if test="${!empty listBankJournal}">
						<c:forEach items="${listBankJournal}" var="bj" varStatus="bjs">
							<tr id="${bj.id}">
								<td class="full"><input type="text" class="fulltd" id="id${bj.id}" name="id" value="${bj.id}" readOnly></td>
								<td class="full"><input type="text" class="date fulltd" id="date${bj.id}" name="date" value="${bj.date}"></td>
								<td class="full"><input id="description${bj.id}" class="fulltd" name="description" value="${bj.description}"></td>
								<td class="full">
									<select class="chosen-select fulltd" id="ietypeId${bj.id}" name="ietypeId" style="width:120px;">
										<option></option>
										<c:if test="${!empty listIEType}">
											<c:forEach items="${listIEType}" var="ie" varStatus="ies">
												<option value="${ie.id}" ${ie.id==bj.ietype.id?'selected':''}>${ie.ie=='0'?'支':'收'}-${ie.name}</option>
											</c:forEach>
										</c:if>
									</select>
								</td>
								<td class="full"><input type="number" class="calculate1 fulltd" id="income${bj.id}" name="income" value="<c:if test='${bj.income!=0}'>${bj.income}</c:if>"></td>
								<td class="full"><input type="number" class="calculate2 fulltd" id="expenditure${bj.id}" name="expenditure" value="<c:if test='${bj.expenditure!=0}'>${bj.expenditure}</c:if>"></td>
								<td class="full"><input type="number" class="balance fulltd" id="balance${bj.id}" name="balance" readonly="readonly"></td>
								<td class="full">
									<a href="javascript:void(0)" onclick="save(${bj.id})">
										<i class="fa fa-save" aria-hidden="true" title="保存"></i>
									</a>
									<a href="javascript:void(0)" onclick="del(${bj.id})">
...
//         'scrollX':true
    ...
    $('#journal').on('blur','#initial',function(){
    	var url='/ssh/cashier/updateBankInitial';
    	var balance=$('#initial').val();
    	var bankId=${bank.id};
    	var args={'balance':balance,'bankId':bankId};
...
var count0=${count};//记录每次auto_increment
function save(trId){
	var id=$('#id'+trId).val();
	var date=$('#date'+trId).val();
	var url='/ssh/cashier/saveBankJournal';
...
//delete ：ajax方式删除一行数据
function del(trid){
	var bankJournalId=trid;
	var table=$('#journal').DataTable;
	url='/ssh/cashier/deleteBankJournal';
	args={'bankJournalId':bankJournalId};
